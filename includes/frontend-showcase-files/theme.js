...
plugins.view.Appearance = wp.Backbone.View.extend({
  ...
  categories: function() {
    ...
    if ( plugins.data.plugins.length === 1 || count_categories < 2 || $('.wmd-plugins-showcase.hide-interface').length ) {
      return;
    }
  ...
  search: function() {
    ...
    if ( plugins.data.plugins.length === 1 || $('.wmd-plugins-showcase.hide-interface').length ) {
      return;
    }
    ...
    view.render();
    $('.plugin-categories')
      .append( $.parseHTML( '<div class="plugin-search-holder"></div>' ) );

    $('.plugin-search-holder')
      .append( $.parseHTML( '<label class="screen-reader-text" for="plugin-search-input">' + l10n.searchFE + '</label>' ) )
      .append( view.el );
  },
...
plugins.view.Plugins = wp.Backbone.View.extend({
  ...
  initialize: function( options ) {
    ...
    this.listenTo( self.collection, 'update', function() {
      ...
      $('.wmd-plugins-showcase').trigger('plugins-list-update');
    });
...
plugins.view.Search = wp.Backbone.View.extend({
  ...
  search: function( event ) {
    ...
    if ( event.target.value ) {
      plugins.router.navigate( plugins.router.baseUrl( '?plugin-search=' + event.target.value ), { replace: true } );
    } else {
...
plugins.view.Categories = wp.Backbone.View.extend({
  ...
  categories: function( event ) {
    ...
    if ( event.target.dataset.category ) {
      ...
      plugins.router.navigate( plugins.router.baseUrl( '?plugin-category=' + event.target.dataset.category ), { replace: true } );
    }
  }
});
  ...
  categories: function( event ) {
    ...
    if ( event.target.href ) {
      ...
      if ( category ) {
        ...
        plugins.router.navigate( plugins.router.baseUrl( '?plugin-category=' + category ), { replace: true } );
      }
    }
  },
...
plugins.Run = {
  ...
  render: function() {
    ...
    if( $('.wmd-plugins-showcase').is("[data-category]") ){
      this.plugins.doCategory( $('.wmd-plugins-showcase').attr('data-category') );
    }
